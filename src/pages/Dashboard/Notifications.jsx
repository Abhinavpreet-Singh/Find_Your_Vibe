      ...
      <div className="pt-24 px-4 max-w-5xl mx-auto">
        ...
        >
          <div className="flex items-center justify-between">
            <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>Notifications</h1>
        ...
        >
          <div className={`flex ${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-md p-1`}>
            <TabButton 
        ...
        {/* Notifications List */}
        <motion.div 
          className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-md overflow-hidden`}
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
...
const NotificationItem = ({ notification, onMarkAsRead, onDelete, isDarkMode }) => {
    ...
    switch (notification.type) {
        ...
        return (
          <>
            <div className="flex items-center">
              ...
              <div>
                <span className={`font-medium ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>{data.user.name}</span>
                {' '}
                <span className={isDarkMode ? 'text-gray-300' : 'text-gray-600'}>{data.message}</span>
              </div>
            </div>
            {notification.type === 'connection_request' && (
              <div className="mt-3 flex space-x-2">
                ...
                <button className={`px-3 py-1.5 border ${isDarkMode ? 'border-gray-600 text-gray-300 hover:bg-gray-700' : 'border-gray-200 text-gray-700 hover:bg-gray-50'} text-sm font-medium rounded-lg`}>
                  Ignore
                </button>
              </div>
            )}
          ...
          <>
            <div className="flex items-center">
              ...
              <div>
                ...
                <span className={isDarkMode ? 'text-gray-300' : 'text-gray-600'}>{data.message}:</span>
                {' '}
                <span className="font-medium text-blue-500">{data.event.title}</span>
                {' '}
                <span className={isDarkMode ? 'text-gray-400' : 'text-gray-500'}>({data.event.time})</span>
              </div>
            </div>
            <div className="mt-3 flex space-x-2">
              <button className="px-3 py-1.5 bg-gradient-to-r from-[#a477ab] to-[#c36376] text-white text-sm font-medium rounded-lg hover:opacity-90">
      ...
      case 'message':
        return (
          <>
            ...
            {data.preview && (
              <div className={`mt-2 p-3 ${isDarkMode ? 'bg-gray-700' : 'bg-gray-50'} rounded-lg ${isDarkMode ? 'text-gray-300' : 'text-gray-600'} text-sm italic`}>
                "{data.preview}"
              </div>
            )}
            <div className="mt-3">
      ...
      case 'connection_suggestion':
        return (
          <div>
            <div className="mb-2">
              <span className={`font-medium ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>{data.users.length}</span>
              {' '}
              <span className={isDarkMode ? 'text-gray-300' : 'text-gray-600'}>{data.message}</span>
            </div>
  ...
  return (
    <div 
      className={`p-4 ${
        isDarkMode 
          ? (!notification.read ? 'bg-purple-900/10' : '') + ' hover:bg-gray-700/50'
          : (!notification.read ? 'bg-purple-50/30' : '') + ' hover:bg-gray-50'
      ...
      <div className="flex">
        ...
        <div className="absolute top-4 right-4 flex items-center">
          <div className="relative">
            ...
            {showActions && (
              <div className={`absolute right-0 mt-1 w-40 ${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-md shadow-lg z-10 py-1`}>
                {!notification.read && (
                  <button 
                    onClick={() => {
...
const EmptyState = ({ isDarkMode }) => {
  return (
    <div className="py-16 flex flex-col items-center justify-center">
      <div className={`h-16 w-16 ${isDarkMode ? 'bg-gray-700' : 'bg-[#a477ab]/10'} rounded-full flex items-center justify-center mb-4`}>
        <FiBell size={32} className={`${isDarkMode ? 'text-[#c36376]' : 'text-[#a477ab]'}`} />
      </div>
      <h3 className={`text-xl font-semibold mb-2 ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}`}>No notifications</h3>
      <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'} text-center max-w-sm mb-6`}>
